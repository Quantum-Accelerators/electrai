...
def apply_hyperparameters(cfg_dict: dict, params: dict) -> dict:
    ...
    if "bottleneck_dilations" in params:
        val = params["bottleneck_dilations"]
        if val == "none":
            cfg["model"].pop("bottleneck_dilations", None)
        else:
            cfg["model"]["bottleneck_dilations"] = [int(x) for x in val.split(",")]
