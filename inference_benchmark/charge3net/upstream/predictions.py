...
def split_batch(batch, max_predict_batch_probes):
    ...
    for i, length in enumerate(section_lens):
        ...
        section_probe_edges[:, :, 1] -= (
            start_idx  # need to reindex to match probe_xyz and displacement
        )
...
def compute_nmape_components(pred, target):
    ...
    return abs_diff.sum().cpu().numpy().item(), torch.abs(
        target
    ).sum().cpu().numpy().item()
...
def compute_nmape(pred, target):
    ...
    return abs_diff.sum() / np.abs(target).sum() * 100.0
...
def save_preds(preds, save_dir):
    ...
    nmape_forumla = lambda x: x["diff_sum"] / x["targ_sum"] * 100.0
...
def combine_partial_cubes(partial_dir, save_dir):
    ...
    partial_files = sorted(
        [(f.stem.split("_"), f) for f in partial_dir.iterdir() if "pred" in f.stem]
    )
