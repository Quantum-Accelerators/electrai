...
class GraphConstructor(object):
    ...
    def atoms_and_probes_to_graph(self, atoms, probe_pos):
        atom_edges, atom_edges_displacement, neighborlist, inv_cell_T = (
            self.atoms_to_graph(atoms)
        )

        probe_edges, probe_edges_displacement = self.probes_to_graph(
            atoms, probe_pos, neighborlist=neighborlist, inv_cell_T=inv_cell_T
        )

        if self.sorted_edges:
            # Sort probe edges for reproducibility
            concat_pe = _sort_by_rows(
                np.concatenate((probe_edges, probe_edges_displacement), axis=1)
            )
...
class KdTreeGraphConstructor(GraphConstructor):
    ...
    def probes_to_graph(self, atoms, probe_pos, neighborlist=None, inv_cell_T=None):
        ...
        supercell_atom_idx = supercell_atom_idx.reshape(
...
class AsapAseGraphConstructor(GraphConstructor):
    ...
    def probes_to_graph(self, atoms, probe_pos, neighborlist=None, inv_cell_T=None):
        try:
            ...
            warnings.warn(
                "Could not import asap3, falling back to ase. See README for installation instructions"
            )
        ...
        if hasattr(neighborlist, "get_neighbors_querypoint"):
            ...
            split_idx = (
                np.ceil(num_probes / self.probe_batch_size)
            ...
            for probe_position in probe_positions:
                ...
                probe_atoms = ase.Atoms(
                    numbers=[0] * n_batch_probes, positions=probe_position
                )
                ...
                if use_ase:
                    ...
                    neighborlist = asap3.FullNeighborList(
                        self.cutoff, atoms_with_probes
                    )

                results.extend(
                    [
                        neighborlist.get_neighbors(i + len(atoms), self.cutoff)
                        for i in range(n_batch_probes)
                    ]
                )
